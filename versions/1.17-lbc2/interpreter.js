...
new FS3Method("fromASCIICode", ["number"], [], (parent, args, interpreter) => {
    let code = parent;

    if(code.value < 0 || code.value > 127){
        throw new FS3Error("RangeError", `ASCII code [${code.value}] must be between 0 and 127`, parent);
    }
...
new FS3Method("xor", ["number"], [{type: ["number"], optional: false}], (parent, args, interpreter) => {
    parent.value = parent.value ^ args[0].value;
    return parent;
});
