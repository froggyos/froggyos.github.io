...
async function sendCommand(command, args = [], createEditableLineAfter = true){
    ...
    switch(command){
        ...
        case '[[BULLFROG]]greeting': {
            ...
            createTerminalLine("This is a special version of FroggyOS for LBC2.", "", {translate: false});
            createTerminalLine("This version has no state saving features so do not close the tab!", "", {translate: false});
            createTerminalLine("FroggyOS was made for Chromium-based browsers first, so if you encounter anything that could be considered a bug, switch to Chrome, etc.", "", {translate: false});
            createTerminalLine("Read the documentation, and good luck!", "", {translate: false});
            printLn();
        } break;
...
function normalizeIndentation(string, tabSize = 4){
